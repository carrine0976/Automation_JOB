...
def input_player_name(customerId:int,name:str,player:str):
    token=get_token()
    logging.info(f"傳入的用戶名稱:{name}")
    API_URL=f"http://sit-admin2.tcg.com/tac/api/relay/post/mcs-player-security-information-changePayeeName?customerId={customerId}&merchantCode=gi8viet&newPayeeName={name}&remark=ggg&updateCard=true"
    headers={
        "Accept": "application/json, text/plain, */*",
        "Accept-Language": "en-US,en;q=0.9",
        "Authorization": token,
    ...
    try:
        ...
        response_data=response.json()
        if response_data.get("success") == True:
            logging.info(f"用戶名輸入成功")  
        else:
            logging.error("用戶名輸入失敗")
            return False
    
    except Exception as e:
        logging.error("用戶名輸入請求失敗")
def apply_bank_card(customerId:int,player:str,name:str,card_number:int):
    token=get_token()
    API_URL3=f"http://sit-admin2.tcg.com/tac/api/relay/post/mcs-player-security-information-createBankCard-BC"
